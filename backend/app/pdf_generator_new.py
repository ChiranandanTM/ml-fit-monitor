"""PDF generator using Canvas API for clean text rendering"""
...
class PDFReportGenerator:
    ...
    def generate_report(self, data):
        ...
        if models:
            ...
            c.setLineWidth(1)
            c.rect(self.margin, y - 15, 5*inch, 15, fill=1, strokeColor=colors.HexColor('#1e40af'), fillColor=colors.HexColor('#1e40af'))
            ...
            for i, m in enumerate(models):
                ...
                if i % 2 == 1:
                    ...
                    c.rect(self.margin, y - row_height, 5*inch, row_height, fill=1, stroke=0)
                    c.setFillColor(colors.black)
        ...
        if features:
            ...
            c.setFont("Helvetica-Bold", 9)
            c.setFillColor(colors.white)
            c.rect(self.margin, y - 15, 3*inch, 15, fill=1, strokeColor=colors.HexColor('#1e40af'), fillColor=colors.HexColor('#1e40af'))
            ...
            for i, f in enumerate(features[:10]):
                if i % 2 == 1:
                    c.setFillColor(colors.HexColor('#f0f4f8'))
                    c.rect(self.margin, y - row_height, 3*inch, row_height, fill=1, stroke=0)
                    c.setFillColor(colors.black)
        ...
        if cm and len(cm) == 2:
            ...
            # Header row
            c.rect(self.margin, y - cell_height, cell_width * 3, cell_height, fill=1, strokeColor=colors.HexColor('#1e40af'), fillColor=colors.HexColor('#1e40af'))
            ...
            for i, label in enumerate(labels):
                c.rect(self.margin, y - cell_height, cell_width * 3, cell_height, fill=0, strokeColor=colors.grey)
